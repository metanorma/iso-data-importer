...
RSpec.describe Iso::Data::Importer::Orchestrator do
  ...
  let(:mock_deliverables_collection) { instance_double(Iso::Data::Importer::Models::DeliverableCollection, "DeliverablesCollection") }
  let(:mock_tc_collection) { instance_double(Iso::Data::Importer::Models::TechnicalCommitteeCollection, "TechnicalCommitteesCollection") }
  let(:mock_ics_collection) { instance_double(Iso::Data::Importer::Models::IcsEntryCollection, "IcsEntriesCollection") }
  ...
  let(:mock_exporter_instance) { instance_double(Iso::Data::Importer::Exporter, "ExporterInstance") }
  ...
  describe '#run_all' do
    ...
    context 'when Scrapers.fetch_all raises an error' do
      ...
      it 'logs a fatal error message containing the error details' do
        ...
        expect(orchestrator).to receive(:log).with(a_string_matching(/FATAL ERROR.*Simulated fetch error/i), :error)
        # Optionally, check that a backtrace log is also attempted (content doesn't need to be exact)
        expect(orchestrator).to receive(:log).with(a_string_matching(/Backtrace/i), :error)
    ...
    context 'when an Exporter method (e.g., clean_output_files) raises an error' do
      let(:export_error) { StandardError.new("Simulated exporter cleaning error") }
      before do
        allow(Iso::Data::Importer::Scrapers).to receive(:fetch_all).and_return(mock_fetched_data)
        allow(Iso::Data::Importer::Exporter).to receive(:new).and_return(mock_exporter_instance)
      ...
      it 'logs a fatal error message containing the error details' do
        expect(orchestrator).to receive(:log).with(a_string_matching(/FATAL ERROR.*Simulated exporter cleaning error/i), :error)
        expect(orchestrator).to receive(:log).with(a_string_matching(/Backtrace/i), :error)
        orchestrator.run_all
      end
